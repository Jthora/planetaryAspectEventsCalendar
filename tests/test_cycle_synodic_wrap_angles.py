...
def test_synodic_normalizes_360_to_zero(monkeypatch):
    ...
    def longitude(body, dt):
        ...
        if body == "A":
            return 0.0
        raw = -10.0 + 20.0 * progress  # -10° to +10° crossing 0
        return raw + 360.0 if raw < 0 else raw
    ...
    cycle_config = CycleConfig(
        ...
        phase_angles=[0.0, 360.0],
    )

    events = detect_synodic_phases(eph, ts, start, end, gen_config, cycle_config, metrics={})

    assert len(events) == 1
    assert events[0].phase_angle == 0.0
    assert abs((events[0].start_time_utc - datetime(2025, 1, 1, 1, 30)).total_seconds()) < 60
