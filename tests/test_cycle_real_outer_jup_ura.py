...
FIXTURE = Path(__file__).resolve().parent / "fixtures" / "real_cycles" / "outer_jup_ura_2024q2.json"
...
def test_outer_jupiter_uranus_window_matches_fixture():
    ...
    cycle_config = CycleConfig(
        ...
        phase_angles=meta["phase_angles"],
    ...
    gen_config = GeneratorConfig(
        ...
        aspect_degrees={},
        planets=planets,
        coarse_step_mins=meta.get("coarse_step_mins", 120),
        refine_step_mins=meta.get("refine_step_mins", 10),
        ...
        verbose=False,
        ascii_only=True,
        retrograde_probe_hours=meta.get("retro_probe_hours", 24.0),
    ...
    for ev, exp in zip(events_sorted, expected_sorted):
        ...
        assert ev.body == exp.get("body")
        assert ev.body1 == exp.get("body1")
        assert ev.body2 == exp.get("body2")
